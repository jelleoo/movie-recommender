...
// 자기 자신은 비교 대상에서 제외하고,
// 유사도 점수가 높은 순서대로 정렬한 뒤 상위 k명만 반환한다.
std::vector<std::pair<std::string, double>> Recommender::findSimilarUsers(
    ...
    // 추천 대상 사용자의 평점이 없으면 비교할 데이터가 없으므로 빈 결과 반환
    if (myRatings.empty()) {
        return similarities;
    }

    for (const std::string& otherId : ratingMgr.getAllUserIds()) {
        // 자기 자신과는 유사도를 계산하지 않는다.
        ...
        // 두 사용자의 평점 목록을 비교하여 유사도 점수를 계산한다.
        double sim = SimilarityCalculator::calculate(myRatings, otherRatings);

        // 유사도가 양수인 사용자만 추천에 사용한다.
        // 공통 영화가 없으면 SimilarityCalculator에서 -100을 반환하므로 제외된다.
        if (sim > 0) {
            similarities.push_back({otherId, sim});
        }
    }

    // pair의 second 값은 유사도 점수이다.
    // 유사도가 큰 사용자가 앞에 오도록 람다로 내림차순 정렬한다.
    ...
    // 유사 사용자가 k명보다 많으면 상위 k명만 남긴다.
    // k명보다 적으면 있는 만큼만 사용한다.
...
std::vector<Movie> Recommender::recommend(
    ...
    // 평점이 하나도 없는 사용자는 취향 비교가 불가능하므로 추천하지 않는다.
    if (myRatings.empty()) {
        return result;
    }
    ...
    // 유사 사용자가 없으면 추천 후보를 만들 수 없으므로 빈 결과 반환
    if (similarUsers.empty()) {
        return result;
    }

    // 내가 이미 본 영화 ID를 저장한다.
    // set은 중복을 허용하지 않고 find()로 빠르게 탐색할 수 있어
    // 추천 후보에서 이미 본 영화를 제외할 때 사용한다.
...
// 기존 추천 알고리즘에 장르 조건을 추가한 함수이다.
// 유사 사용자 탐색 방식은 recommend()와 같고,
// 후보 영화 수집 단계에서 입력한 장르와 일치하는 영화만 남긴다.
std::vector<Movie> Recommender::recommendByGenre(
